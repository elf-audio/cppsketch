...
public:
	ofxCppSketch(string className, string mainFilePath)
		: ofBaseApp() {
...
protected:
	std::filesystem::path getLibsPath() { return srcDir.parent_path() / OF_PATH / "libs"; }
	std::filesystem::path getAddonsPath() { return srcDir.parent_path() / OF_PATH / "addons"; }

	std::filesystem::path getOfLibPath() { return getLibsPath() / "openFrameworks"; }
	...
	bool isIncludePath(string path) {
		...
		return (incPos == path.size() - 8) || (incPos == path.size() - 9 && path[path.size() - 1] == '/');
	}
